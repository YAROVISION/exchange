...
class CustomKlineDataset(Dataset):
    ...
    def _split_data_by_time(self):
        ...
        if max_start < 0:
            self.n_samples = 0
            self.start_indices = []
            print(f"[{self.data_type.upper()}] Warning: Dataset length ({total_len}) is smaller than window size ({self.window})!")
        else:
            total_samples = max_start + 1
            train_count = int(total_samples * self.train_ratio)
            val_count = int(total_samples * self.val_ratio)
            
            # Ensure at least 1 sample if ratio > 0 and total_samples > 0
            if train_count == 0 and self.train_ratio > 0:
                train_count = min(1, total_samples)
            if val_count == 0 and self.val_ratio > 0:
                val_count = min(1, total_samples - train_count)
                
            test_count = total_samples - train_count - val_count
            
            if self.data_type == 'train':
                self.n_samples = train_count
                self.start_indices = list(range(0, train_count))
            elif self.data_type == 'val':
                self.n_samples = val_count
                self.start_indices = list(range(train_count, train_count + val_count))
            elif self.data_type == 'test':
                self.n_samples = test_count
                self.start_indices = list(range(train_count + val_count, total_samples))
        
        print(f"[{self.data_type.upper()}] Data length: {len(self.data)}, Available samples: {self.n_samples}")
        if self.n_samples > 0:
            first_idx = self.start_indices[0]
            last_idx = self.start_indices[-1] + self.window - 1
            print(f"[{self.data_type.upper()}] Time range: {self.timestamps.iloc[first_idx]} to {self.timestamps.iloc[last_idx]}")
    ...
    def __getitem__(self, idx):
        if self.n_samples <= 0:
            raise ValueError(f"Insufficient data to create samples for split '{self.data_type}'")
            
        if self.data_type == 'train':
            epoch = getattr(self, 'current_epoch', 0)
            # Shuffle start indices within the train split
            perm_idx = (idx * 9973 + (epoch + 1) * 104729) % self.n_samples
            start_idx = self.start_indices[perm_idx]
        else:
            start_idx = self.start_indices[idx % self.n_samples]
...
def train_model(model, tokenizer, device, config, save_dir, logger, start_epoch=0):
    ...
    scheduler_epochs = config.basemodel_epochs - start_epoch
    if scheduler_epochs <= 0:
        raise ValueError(f"basemodel_epochs ({config.basemodel_epochs}) must be greater than start_epoch ({start_epoch})")
...
def main():
    ...
    parser.add_argument('--start-epoch', type=int, default=0,
                       help='Epoch to start/resume training from (default: 0)')
    parser.add_argument('--basemodel-epochs', type=int, default=None,
                       help='Override total number of basemodel epochs')
    parser.add_argument('--pretrained-predictor', type=str, default=None,
                       help='Override path to pretrained predictor model checkpoint')
    parser.add_argument('--device', type=str, default=None,
                       help='Override target device (e.g. cpu, mps, cuda)')
    args = parser.parse_args()
    
    config = CustomFinetuneConfig(args.config)
    
    # Override settings if arguments are provided
    if args.basemodel_epochs is not None:
        config.basemodel_epochs = args.basemodel_epochs
    if args.pretrained_predictor is not None:
        config.pretrained_predictor_path = args.pretrained_predictor
        config.pre_trained_predictor = True
    ...
    best_val_loss = train_model(model, tokenizer, device, config, config.basemodel_save_path, logger, start_epoch=args.start_epoch)
