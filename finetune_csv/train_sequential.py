...
class SequentialTrainer:
    ...
    def __init__(self, config_path: str = None, args = None):
        ...
        # Override config settings from args if provided
        if args is not None:
            if getattr(args, 'basemodel_epochs', None) is not None:
                self.config.basemodel_epochs = args.basemodel_epochs
            if getattr(args, 'pretrained_predictor', None) is not None:
                self.config.pretrained_predictor_path = args.pretrained_predictor
                self.config.pre_trained_predictor = True
    ...
    def _setup_device(self):
        if self.args is not None and getattr(self.args, 'device', None) is not None:
    ...
    def train_basemodel_phase(self):
        ...
        start_time = time.time()
        start_epoch = self.args.start_epoch if self.args is not None else 0
...
def main():
    ...
    parser.add_argument('--data-path', type=str, default=None,
                       help='Override data path for training')
    
    args = parser.parse_args()
    
    trainer = SequentialTrainer(args.config, args)
    
    if args.data_path:
        trainer.config.data_path = args.data_path
