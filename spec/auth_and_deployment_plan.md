...
4. **Ендпоінти гри «Біржа»:**
   * `/api/exchange/wallet` (GET) — отримання балансу USD/BTC та середньої ціни з таблиці `wallets`. Вимагає авторизації (інакше повертає `401 Unauthorized`).
   * `/api/exchange/trade` (POST) — виконання ордерів купівлі/продажу. Вимагає авторизації (інакше повертає `401 Unauthorized`):
   ...
   * `/api/exchange/reset` (POST) — скидання балансу гаманця до $100.00 та очищення історії транзакцій. Вимагає авторизації.
5. **Обмеження прав доступу на бекенді (Ролі):**
   * Ендпоінти запуску та зупинки донавчання моделі (`/api/finetune/start`, `/api/finetune/stop`) мають містити перевірку email авторизованого користувача. Якщо email не дорівнює `yarovision@gmail.com`, сервер повертає статус `403 Forbidden` і відхиляє запит.
...
2. **Прив'язка інтерфейсу та кнопок до стану входу й ролей:**
   * **Незареєстровані / неавторизовані користувачі:**
     - Усі кнопки керування на вкладці «🏦 Біржа» (кнопки перемикання режиму ордера, кнопка виконання угоди, кнопки регулювання суми `+`/`-`, кнопка скидання балансу) робляться неактивними (`disabled`).
     - Усі кнопки керування на вкладці «⚙️ Донавчання» (кнопка запуску донавчання, кнопка зупинки процесу) також робляться неактивними (`disabled`).
     - Поруч відображається повідомлення-підказка про те, що для виконання операцій потрібно увійти в систему.
   * **Авторизовані звичайні користувачі:**
     - Кнопки на вкладці «🏦 Біржа» стають повністю активними.
     - Кнопки на вкладці «⚙️ Донавчання» залишаються неактивними (`disabled`) з підказкою: *«Доступно лише для головного адміністратора»*.
   * **Головний адміністратор (`yarovision@gmail.com`):**
     - Всі кнопки на обох вкладках («🏦 Біржа» та «⚙️ Донавчання») є повністю активними.
